...
The module is **vendored**, not shared: ``src-tauri/crates/diag``. Every repo in the fleet
...
| Platform | Directory |
| --- | --- |
| macOS | `~/Library/Logs/av-launcher/` |
| Linux | `$XDG_STATE_HOME/av-launcher/logs/` (default `~/.local/state/...`) |
| Windows | `%LOCALAPPDATA%\av-launcher\logs\` |

`AV_LAUNCHER_LOG_DIR` overrides it. The path is printed on the first line of every
...
`av-launcher.YYYY-MM-DD.log`, rotated daily, seven kept, no colour escapes.

Verbosity comes from `AV_LAUNCHER_LOG` (falling back to `RUST_LOG`), default `info`.
...
`av-launcher-crash-<timestamp>.json`; the path is printed on the way down. The
...
```bash
av-launcher --collect-diagnostics
```

Writes `av-launcher-diagnostics-<timestamp>.json` and prints its path. One file, so
